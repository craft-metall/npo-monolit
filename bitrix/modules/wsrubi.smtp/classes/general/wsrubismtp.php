    ...
    function custom_mail($to, $subject, $message, $additional_headers = null, $additional_parameters = null)
    {
        ...
        if (!empty($additional_headers)) {
            try {
                ...
                WsRubiTools::WsrubiSMTPLog($e->getMessage() . "\tline:" . __LINE__ . "\t" . $additional_headers, 'error');
            }

            try {
                $Message->setHeaders($oHeaders);
            } catch (\Exception $e) {
                WsRubiTools::WsrubiSMTPLog($e->getMessage() . "\tline:" . __LINE__ . "\t", 'error');
            }
        ...
        foreach ($arrTo as $to) {
            try {
                ...
                WsRubiTools::WsrubiSMTPLog($to . "-" . $e->getMessage() . "\tline:" . __LINE__, 'error');
            }
        }
        if(empty($sFrom))
        ...
        try {
            ...
            WsRubiTools::WsrubiSMTPLog("from-'" . $sFrom . "\t" . $e->getMessage() . "\t" . "\tline:" . __LINE__, 'error');
            return false;
        }
        try {
            $emailTo = $to;
            ...
            WsRubiTools::WsrubiSMTPLog("from-'" . $sFrom . "'\tto-'" . $emailTo . "'\t" . $e->getMessage() . "\t" . $Message->toString() . "\t" . "\tline:" . __LINE__, 'error');
            return false;
        }
        try {
            ...
            WsRubiTools::WsrubiSMTPLog("from-'" . print_r($Message->getFrom(), 1) . "'\tto-'" . $emailTo . "'\t" . $e->getMessage() . "\t" . $Message->toString() . "\t" . "\tline:" . __LINE__, 'error');
            return false;
        }
    }
}

...
function insert_text_cursor(textarea, macro) {
	var area = BX(textarea);

	if ((area.selectionStart) || (area.selectionStart == '0')) {
		var p_start = area.selectionStart;
		var p_end = area.selectionEnd;

		area.value = area.value.substring(0, p_start) + macro + area.value.substring(p_end, area.value.length);
	}

...
?>
	<tr>
		<td width="40%"><?echo GetMessage("sender_contact_edit_field_type")?></td>
		<td width="60%">
			<select name="TYPE_ID">
				<?foreach (\Bitrix\Sender\Recipient\Type::getNamedList() as $code => $name):?>
					<option value="<?=htmlspecialcharsbx($code)?>" <?=($str_TYPE_ID == $code ? 'selected' : '')?>>
	...
	<tr>
		<td width="40%"><?echo GetMessage("sender_contact_edit_field_code")?></td>
		<td width="60%"><input type="text" name="CODE" value="<?echo $str_CODE;?>" size="45" maxlength="100"></td>
	</tr>

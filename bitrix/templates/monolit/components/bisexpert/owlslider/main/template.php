...
<?if (count($arResult['ITEMS']) > 0):?>
					...
					<?foreach($arResult['ITEMS'] as $item):?>
						<div class="owl-carousel__item">
                            ...
                            <?else:?>
                                ...
                                <?endif;?>
                                    <img class="adaptive-img" title="<?=htmlspecialchars_decode($item['ITEM_TEXT']);?>" alt="<?=htmlspecialchars_decode($item['ITEM_TEXT']);?>" src="<?=$item['PICTURE_RESIZED']['src'];?>" />
                                <?if ($item['ITEM_URL'] != ''):?>
                                </a>
                                <?endif;?>
                            <?endif;?>
                            ...
                            <div class="owl-carousel__item-descr">
                                <div class="slide-title"><?=$item['NAME'];?></div>
								<div class="slide-text<?if ($item['PROPERTY_2'] == ''):?> p3<?endif;?>">
                                <?if ($item['NAME'] != $item['ITEM_TEXT']) {
                                    echo htmlspecialchars_decode($item['ITEM_TEXT']);;
                                }
                                ?></div>
								<?if ($item['PROPERTY_2'] != ''):?>
									<a class="slide-button" href="<?=$item['PROPERTY_2']?>" title="<?=$item['PROPERTY_3']?>">
										<?=$item['PROPERTY_3']?>
									</a>
								<?endif;?>
                            </div>
    ...
    <script type="text/javascript">
		if (/iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) {
			document.querySelector('.owl-carousel__item-link').removeChild(document.querySelector('.adaptive-img'));
		}
        (function ($) {
            $(function() {
